...
(function () {
  ...
  var rainbow = function rainbow() {
    ...
    var sin_to_hex = function sin_to_hex(i, phase) {
      var sin = Math.sin(Math.PI / size * 2 * i + phase);

      var _int = Math.floor(sin * 127) + 128;
    ...
    function draw() {
      ...
      if (clear) {
        ...
        */
        ...
        var shadowOffset = ctx.shadowOffsetX < ctx.shadowOffsetY ? ctx.shadowOffsetX : ctx.shadowOffsetY;
  ...
  (function () {
    ...
    function () {
      ...
      _createClass(Branch, [{
        ...
        value: function draw() {
          ...
          ctx.save(); //ctx.fillStyle = pallet[this.depth % pallet.length];
        ...
        key: "killBranchIfTooSmall",
        value: function killBranchIfTooSmall() {
          if (this.radius < 0.2) {
            branchCollection.remove(this);
        ...
        key: "drawLeaf",
        value: function drawLeaf(x, y) {
          var ctx = this.context; //ctx.strokeStyle = pallet[this.depth  % pallet.length];
    ...
    function setup() {
      var $canvas = $(canvas.el); // Dimensions

      var width = $canvas.width(); // Branches

      var n = 4 + Math.random() * 8;
      var initialRadius = width / 50;

      for (var i = 0; i < n; i++) {
        var branch = new Branch($canvas);
